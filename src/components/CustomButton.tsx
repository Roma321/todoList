...
export const CustomButton = ({ onPress, title, width, type }: Props) => {
  const height = width / 121 * 36;
  return (
    <TouchableOpacity style={styles.buttonContainer} onPress={onPress}>
      <Svg width={width} height={height} viewBox="0 0 121 36">
        <Path
          d={type === 'outlined' ? OUTLINED_PATH : SOLID_PATH}
          fill={type === 'solid' ? PRIMARY_COLOR : 'transparent'}
          stroke={type === 'outlined' ? PRIMARY_COLOR : 'transparent'}
        ...
        <Text
          ...
          textAnchor="middle"
          fill={type === 'solid' ? 'white' : PRIMARY_COLOR}
          fontSize="16"
          fontWeight="bold"
        >
          {title}
