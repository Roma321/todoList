...
export const CustomButton = ({ onPress, title, width, height, type, color, fontSize, textColor }: Props) => {
  const scaleX = width / BASE_WIDTH;
  const scaleY = height / BASE_HEIGHT;
  const textColorFill = textColor ?? (type === 'solid' ? theme.colors.background : color);
  ...
  return (
    <TouchableOpacity style={styles.buttonContainer} onPress={onPress}>
      <Svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        />
        <Text
          x={BASE_WIDTH / 2 * scaleX}
          y={BASE_HEIGHT / 2 * scaleY}
